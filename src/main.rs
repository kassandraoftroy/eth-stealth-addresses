...
fn encode_stealth_meta_address(
    pks: k256::ProjectivePoint,
    pkv: k256::ProjectivePoint,
) -> StealthMetaAddress {
...
fn decode_stealth_meta_address(
...
fn encode_pubkey(x: k256::ProjectivePoint) -> PublicKeyCompressed {
    let b = x.to_bytes();
...
fn decode_pubkey(encoded: &PublicKeyCompressed) -> k256::ProjectivePoint {
    return k256::ProjectivePoint::from_bytes(GenericArray::from_slice(encoded)).unwrap();
}

fn encode_priv(x: k256::Scalar) -> PrivateKey {
    let b = x.to_bytes();
...
fn decode_priv(encoded: &PrivateKey) -> k256::Scalar {
...
fn get_address_from_pubkey(pubkey: k256::ProjectivePoint) -> Address {
